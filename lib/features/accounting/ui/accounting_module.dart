...
/// Accounting Module — Self-contained UI shell + demo data (no backend yet)
///
/// Sections:
/// - Daily Sales Summary & Reconciliation (by payment modes, status chip)
/// - GSTR-1 & GSTR-3B (tabs with summary tables)
/// - Profit & Loss (P&L) with simple bar chart
/// - Manual Expenses (list with add button)
/// - Tax & Liability Summary (GST payable, ITC, Net Liability)
/// - Export Actions (CSV/Excel placeholders)
/// - Outstanding Credits (customers with pending credits)
/// - Cash Book / Day Book (ledger entries with running balance)
///
/// TODOs are provided for wiring to real backend later (Firestore/Functions or APIs).
...
class _AccountingModuleScreenState extends State<AccountingModuleScreen> with SingleTickerProviderStateMixin {
  // Demo sales split by payment mode
  ...
  bool get _reconciled => true; // demo flag
  ...
  Widget build(BuildContext context) {
    ...
    final salesCard = _DailySalesCard(
      salesByMode: _salesByMode,
      total: _todayTotal,
      reconciled: _reconciled,
    );

    final gstrCard = _GstReportsCard(
...
class _GstReportsCard extends StatelessWidget {
  ...
  final List<_GstRow> gstr1;
  final List<_GstRow> gstr3b;
    ...
    return DataTable(columns: const [
      ...
      for (final r in rows)
        DataRow(cells: [
          ...
          DataCell(Text('₹${r.taxable.toStringAsFixed(0)}')),
      ...
      child: Padding(
        ...
        child: Column(children: [
          TabBar(
            ...
            tabs: const [Tab(text: 'GSTR-1'), Tab(text: 'GSTR-3B')],
          ),
          SizedBox(
            height: 220,
            child: TabBarView(
              controller: tabController,
              children: [
                SingleChildScrollView(child: _table(gstr1)),
                SingleChildScrollView(child: _table(gstr3b)),
              ],
            ),
          ),
        ]),
...
class _OutstandingCreditsCard extends StatelessWidget {
  ...
  const _OutstandingCreditsCard({required this.credits});
  ...
  Widget build(BuildContext context) {
    ...
    return Card(
      child: Padding(
        padding: const EdgeInsets.all(12),
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text('Cash Book / Day Book', style: Theme.of(context).textTheme.titleMedium),
          const SizedBox(height: 8),
          DataTable(columns: const [
            ...
            for (final row in rows)
              DataRow(cells: [
                DataCell(Text('${row.$1.date.toLocal()}')),
                DataCell(Text(row.$1.desc)),
                DataCell(Text(row.$1.inAmt == 0 ? '-' : '₹${row.$1.inAmt.toStringAsFixed(0)}')),
                DataCell(Text(row.$1.outAmt == 0 ? '-' : '₹${row.$1.outAmt.toStringAsFixed(0)}')),
                DataCell(Text('₹${row.$2.toStringAsFixed(0)}')),
              ]),
          ]),
        ]),
      ),
...
class _TaxSummary {
  ...
  final double itc;
  const _TaxSummary({required this.gstPayable, required this.itc});
}
...
class _LedgerEntry {
  final DateTime date;
  final String desc;
  final double inAmt; // cash in
  final double outAmt; // cash out
