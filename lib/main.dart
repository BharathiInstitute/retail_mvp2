...
Future<void> main() async {
  ...
  try {
    ...
    final msg = error.toString();
    if (_isBenignWebFirestoreUnsubscribeError(msg)) {
      // Ignore benign async errors from Firestore web interop
      return;
    }
    // ignore: avoid_print
...
bool _isBenignWebFirestoreUnsubscribeError(String message) {
  ...
  final looksLikeFirestore = m.contains('cloud_firestore_web') ||
      m.contains('firebase.firestore') ||
      m.contains('interop/firestore.dart') ||
      (m.contains('firestore') && (m.contains('snapshot') || m.contains('onsnapshot') || m.contains('unsubscribe')));
  if (!looksLikeFirestore) return false;
  // Common signatures across plugin versions
  if (m.contains('onsnapshotunsubscribe') ||
      m.contains('onsnapshot') && m.contains('unsubscribe') ||
      m.contains('callasfunction') ||
      m.contains('callmethod') ||
      m.contains('_createsnapshotstream')) {
    return true;
  }
  // Dart-side stream/controller races
  if (m.contains('lateinitializationerror') && m.contains('controller') && m.contains('snapshot')) return true;
  if (m.contains('bad state') && m.contains('stream') && m.contains('already been listened')) return true;
  // Occasional JS type errors bubbling up from interop
  if (m.contains('typeerror') && (m.contains('unsubscribe') || m.contains('onsnapshot') || m.contains('callasfunction') || m.contains('callmethod'))) return true;
  return false;
}
